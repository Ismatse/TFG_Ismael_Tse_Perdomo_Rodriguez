...
class LPU(nn.Module):
    ...
    def forward(self, x):
        ...
        result = (self.depthwise(x) + x).transpose(1,2).transpose(2, 3).contiguous()  # B H*W C
        return result
...
class PosCNN(nn.Module):
    ...
    def forward(self, x, H=None, W=None):
        ...
        feat_token = x
        cnn_feat = feat_token.transpose(1, 2).transpose(1, 3)
        if self.s == 1:
...
class Downsample(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.transpose(1, 2).transpose(1, 3).contiguous()
        out = self.conv(x).transpose(1,2).transpose(2, 3).contiguous()  # B H*W C
        return out
...
class Upsample(nn.Module):
    ...
    def forward(self, x):
        B, H, W, C = x.shape
        x = x.transpose(1, 2).transpose(1, 3).contiguous()
        out = self.deconv(x).transpose(1, 2).transpose(2, 3).contiguous() # B H*W C
        return out
...
class InputProj(nn.Module):
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        x = self.proj(x).transpose(1, 3).contiguous()  # B H*W C
...
class LeWinTransformerBlock(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        if mask != None:
            ...
            input_mask_windows = window_partition(input_mask, self.win_size) # nWindows, win_size, win_size, 1
            attn_mask = input_mask_windows.view(-1, self.win_size * self.win_size) # nWindows, win_size*win_size
            attn_mask = attn_mask.unsqueeze(2)*attn_mask.unsqueeze(1) # nWindows, win_size*win_size, win_size*win_size
        ...
        if self.shift_size > 0:
            # calculate attention mask for ShiftedWindowBased-MultiHeadSelfAttention (SW-MSA)
...
class Uformer(nn.Module):
    def __init__(self, img_size_a=256, img_size_b=256, in_chans=3, dd_in=3,
                 ...
                 cross_modulator=False, segmentation=False, num_classes=2,  **kwargs):
        super().__init__()
        ...
        self.mlp = token_mlp
        self.win_size =win_size
        self.reso = [img_size_a, img_size_b]
        ...
        # Output #num_classes channels if it's a segmentation problem
        if segmentation:
            self.output_proj = OutputProj(in_channel=2*embed_dim, out_channel=num_classes, kernel_size=3, stride=1)
        else:
        ...
        self.encoderlayer_2 = BasicUformerLayer(dim=embed_dim*4,
                            ...
                            input_resolution=(img_size_a // (2 ** 2),
                                                img_size_b // (2 ** 2)),
                            depth=depths[2],
        ...
        self.encoderlayer_3 = BasicUformerLayer(dim=embed_dim*8,
                            ...
                            input_resolution=(img_size_a // (2 ** 3),
                                                img_size_b // (2 ** 3)),
                            depth=depths[3],
        ...
        self.conv = BasicUformerLayer(dim=embed_dim*16,
                            ...
                            input_resolution=(img_size_a // (2 ** 4),
                                                img_size_b // (2 ** 4)),
                            depth=depths[4],
    ...
    def flops(self):
        flops = 0
        # Input Projection
        flops += self.input_proj.flops(self.reso[0],self.reso[1])
        # Encoder
        flops += self.encoderlayer_0.flops()+self.dowsample_0.flops(self.reso[0],self.reso[1])
        flops += self.encoderlayer_1.flops()+self.dowsample_1.flops(self.reso[0]//2,self.reso[1]//2)
        flops += self.encoderlayer_2.flops()+self.dowsample_2.flops(self.reso[0]//2**2,self.reso[1]//2**2)
        flops += self.encoderlayer_3.flops()+self.dowsample_3.flops(self.reso[0]//2**3,self.reso[1]//2**3)

        # Bottleneck
        flops += self.conv.flops()

        # Decoder
        flops += self.upsample_0.flops(self.reso[0]//2**4,self.reso[1]//2**4)+self.decoderlayer_0.flops()
        flops += self.upsample_1.flops(self.reso[0]//2**3,self.reso[1]//2**3)+self.decoderlayer_1.flops()
        flops += self.upsample_2.flops(self.reso[0]//2**2,self.reso[1]//2**2)+self.decoderlayer_2.flops()
        flops += self.upsample_3.flops(self.reso[0]//2,self.reso[1]//2)+self.decoderlayer_3.flops()
        
        # Output Projection
        flops += self.output_proj.flops(self.reso[0],self.reso[1])
        return flops
...
if __name__ == "__main__":
    ...
    model_restoration = Uformer(img_size_a=384, img_size_b=640, embed_dim=16,depths=depths,
