...
parser = argparse.ArgumentParser(description='PyTorch HyperKVir Training')
...
parser.add_argument('--get-features', default='', type=str, metavar='PATH',
...
def get_training_data(rgb_dir, transform=None):
    assert os.path.exists(rgb_dir)
    return DataLoaderTrain(rgb_dir, transform)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.distributed:
        ...
        if args.gpu is not None:
            ...
            args.batch_size = int(args.batch_size / n_gpus_total)
    ...
    train_transforms = SimSiamTransform(224, mean_std=hyperKvir_mean_std, N=args.N, M=args.M, grid=args.grid)
    train_dataset = get_training_data(traindir, train_transforms)
    if args.distributed:
        train_sampler = torch.utils.data.distributed.DistributedSampler(train_dataset)
        print(f'Process {dist.get_rank()}, {args.rank}, {gpu}: {len(train_sampler)} training samples per epoch')
    else:
        train_sampler = None
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        logger.info('Lr:{} epoch: {}'.format(optimizer.param_groups[0]['lr'], epoch))
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            }, is_best=False, filename=os.path.join(saved_path, 'checkpoint_latest.pth.tar'.format(epoch)))
        torch.cuda.empty_cache()
...
def train(train_loader, model, optimizer, epoch, args):
    ...
    for i, (images, _) in enumerate(train_loader):
        ...
        z11, p11 = model(images[2])
        z21, p21 = model(images[3])

        loss = negcos(p1, z2) / 2 + negcos(p2, z1) / 2 + negcos(p11, z1) / 2 + negcos(p21, z2) / 2
